...
### 7.索引失效
![mysql的索引失效](/images/article/mysql/mysql-idnex.jpg "索引失效")  
举例：前提条件组合索引（name,age,position）
#### 1.全值匹配我最爱
1.	Select *from staff where name =’xie’  
2.	Select *from staff where name =’xie’ and age = 25  
3.	Select *from staff where name =’xie’ and age = 25 and position = ’man’  
以上查询语句索引都会有效
#### 2.最佳左前缀法则
如果索引了多列，要遵守最左前缀法则。意思是：查询你要从索引的最左前列开始并且不跳过索引的列。
1.	Select *from staff where name =’xie’  
2.	Select *from staff where age = 25  
3.	Select *from staff where age = 25 and position = ’man’  
4.	Select *from staff where name =’xie’ and position = ’man’  
...
Select *from staff where name =’xx’ or age >10索引会不起作用